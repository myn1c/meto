...
3. [Автоматический установщик](#автоматический-установщик)
4. [Вручную](#вручную)
...
METO - это готовая клиент‑серверная система для ESP32:
Клиент (на ESP32) подключается к Wi‑Fi, собирает данные (например от датчиков) и отправляет их на сервер.
Сервер (на Go + Gin) принимает данные через HTTP и предоставляет удобный веб‑интерфейс для просмотра и мониторинга.
Для удобства есть автоматический установщик, который настраивает всё «под ключ» (Wi‑Fi, IP, порт, скачивание клиента и сервера).
Это простой путь превратить ESP32 + датчики в «умную» систему с веб‑панелью, без глубокого погружения в сетевое программирование.
...
Текущие настройки:
1. Название сети (SSID): а
2. Пароль (PASS): а
3. Порт (PORT): 8080
4. Хост (HOST): 169.254.142.85
